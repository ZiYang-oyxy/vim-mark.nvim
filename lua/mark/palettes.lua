...
function M.maximum()
  local palette = copy(M.original)
  local tco = tonumber(vim.go.t_Co) or 256
  local has_truecolor = vim.go.termguicolors
  if has_truecolor or tco >= 88 then
    vim.list_extend(palette, copy(maximum_88))
  end
  if has_truecolor or tco >= 256 then
    vim.list_extend(palette, copy(maximum_256))
  end
  if has_truecolor then
    vim.list_extend(palette, copy(maximum_gui))
  end
  return palette
end
...
M.builtin = {
  original = function()
    return copy(M.original)
  end,
  extended = function()
    return copy(M.extended)
  end,
  maximum = M.maximum,
}

function M.all(user_palettes)
  return vim.tbl_extend("force", copy(M.builtin), user_palettes or {})
end

function M.resolve(name_or_palette, user_palettes)
  if type(name_or_palette) == "table" then
    return copy(name_or_palette)
  end
  local palette_name = name_or_palette or "original"
  local all_palettes = M.all(user_palettes)
  local value = all_palettes[palette_name]
  ...
  return nil, ("Unknown palette: %s"):format(tostring(palette_name))
end
